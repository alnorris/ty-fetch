...
import { fetchSpecForDomain, configuredSpecs, specCache } from "../src/core/spec-cache";
...
`;
    ...
    try {
      ...
      configuredSpecs[domain] = `http://127.0.0.1:${port}/spec.yaml`;
...
`;
    ...
    try {
      const domain = `127.0.0.1:${port}`;
      specCache.delete(domain);
      configuredSpecs[domain] = `http://127.0.0.1:${port}/api-spec`;
      const entry = await fetchSpecForDomain(domain, () => {});
      assert.equal(entry.status, "loaded");
  ...
  it("still parses JSON specs correctly", async () => {
    ...
    try {
      const domain = `127.0.0.1:${port}`;
      specCache.delete(domain);
      configuredSpecs[domain] = `http://127.0.0.1:${port}/spec.json`;
      const entry = await fetchSpecForDomain(domain, () => {});
      assert.equal(entry.status, "loaded");
